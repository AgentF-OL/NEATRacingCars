...
Usage:
    python src/analysis/radars_car_analysis.py
"""
import os
import csv
...
def analyze(results_dir='results/radars'):
    ...
    plt.ylabel('Fitness')
    plt.title('Radar NEAT — Fitness Evolution')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    plt.xlabel('Generation')
    plt.ylabel('Count')
    plt.title('Radar NEAT — Network Complexity')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
